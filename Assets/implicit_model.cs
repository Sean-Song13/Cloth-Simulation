...
public class implicit_model : MonoBehaviour
{
    ...
    void Get_Gradient(Vector3[] X, Vector3[] X_hat, float t, Vector3[] G)
    {
        ...
        {
            G[n] = t_term * mass * (X[n] - X_hat[n]);
            G[n] -= mass * gravity * Vector3.down;
        }

        //Spring Force.
        for (int n = 0, e = 0; n < E.Length; n += 2)
        {
            int i = E[e * 2 + 0];
            int j = E[e * 2 + 1];
            var le = L[e];
            var f = spring_k * (1 - le / (X[i] - X[j]).magnitude) * (X[i] - X[j]);
            G[i] = G[i] + f;
            G[j] = G[j] - f;
            e++;
        }
    ...
    void Update()
    {
        ...
        for (int k = 0; k < 32; k++)
        {
            ...
            for (int n = 0; n < X.Length; n++)
            {
                ...
                var F_2nd = t_term * mass + 4 * spring_k;
                dx = -Mathf.Pow(F_2nd, -1) * G[n];
                var old_X = X[n];
                X[n] = X[n] + dx;
                X[n] = w * X[n] + (1 - w) * last_X[n];
                last_X[n] = old_X;
            }
            if(dx.magnitude < 0.00005f) break;
        }
